...
public class TestAASd_081 {
	...
	public void correctIdType() throws ValidationException {
		Key key = createWithTypeAssetAdministrationShellKey(KeyType.CUSTOM);
	...
	public void wrongIdTypeIdShort() {
		Key key = createWithTypeAssetAdministrationShellKey(KeyType.ID_SHORT);
		try {
			ShaclValidator.getInstance().validate(key);
			fail();
	...
	public void wrongIdTypeFragmentId() {
		Key key = createWithTypeAssetAdministrationShellKey(KeyType.FRAGMENT_ID);
	...
	private Key createWithTypeAssetAdministrationShellKey(KeyType custom) {
